...
//                    HashSet<BluetoothDevice> set = new HashSet<BluetoothDevice>();
//                    for (BluetoothDevice bean : devicesList) {
//                        boolean add = set.add(bean);
//                        if (add) {
//                            mNewDevicesArrayAdapter.add(bean.getName() + "\n" + bean.getAddress());
//                        }
//                    }
//                }
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 注册广播接收器，接收并处理搜索结果
        registerReceiver(mFindBlueToothReceiver, intentFilter);
        initBluetooth();
    }
...
//        mBluetoothAdapter = BluetoothAdapter.getDefaultAdapter();
        ...
        if (pairedDevices.size() > 0) {
            ...
            for (BluetoothDevice device : pairedDevices) {
                ...
                //todo mPairedDevicesArrayAdapter.add(device.getName() + "\n" + device.getAddress());
            }
        } else {
            String noDevices = getResources().getText(R.string.none_paired).toString();
